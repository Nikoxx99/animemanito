...
foreach ($data['episode']['videos'] as $key => $video)
{
    if (isset( $players[ $video['player'] ] )) {
        ...
        $data['episode']['videos'][$key]['code'] = $config['urlpath'].'/redirect.php?player='.$players[ $video['player'] ]['id'].'&code='.urlencode($data['episode']['videos'][$key]['code']).'&thumbnail='.urlencode(getEpisodeImage($data['serie']['image_screenshot'], $data['episode']['image']));
        $data['episode']['videos'][$key]['type'] = $players[ $video['player'] ]['type'];
    }
}

$episode_player_backgroud = getEpisodeImage($data['serie']['image_screenshot'], $data['episode']['image']);
...
$data['episode_recommended'] = DB::query('SELECT id,name,genres,image_cover,url FROM serie WHERE category_id = %s0 and genres like %s1 and genres like %s2 and genres like %s3 ORDER BY RAND() LIMIT 4',1,'%'.$data['serie']['genres'][0].'%','%'.$data['serie']['genres'][1].'%','%'.$data['serie']['genres'][2].'%');
